...
public abstract class StringValueResolverTest extends TestBase {
    ...
    public abstract static class PredefinedTest {
        ...
        public void testEmptyStr() {
            TestBase.eq(empty, _p.resolve(""));
            Assert.assertNull(_r.resolve(""));
        }

        protected void verify(Object expected, String s) {
            TestBase.eq(expected, _p.resolve(s));
            TestBase.eq(expected, _r.resolve(s));
        }

        protected void verifyException(Class<? extends Exception> expected, String s) {
            try {
                _p.resolve(s);
                Assert.assertFalse(S.fmt("Expection %s expected", expected), true);
            } catch (Exception e) {
                if (!expected.isAssignableFrom(e.getClass())) {
                    Assert.assertFalse(S.fmt("Expection %s expected, found: %s", expected, e.getClass()), true);
                }
            }
            try {
                _r.resolve(s);
