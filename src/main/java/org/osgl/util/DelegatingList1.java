 ...
 */
class DelegatingList1<T> extends ListBase<T> implements C.List<T> {

    private final int hcCls_ = getClass().hashCode();

    protected java.util.List<T> data;

    private DelegatingList1() {
    }

    DelegatingList1(Iterable<? extends T> i) {
    ...
    DelegatingList1(Collection<? extends T> c) {
        data = newJavaList(c.size());
        data.addAll(c);
    }

    DelegatingList1(java.util.List<T> list, boolean noCopy) {
        if (noCopy) {
            data = list;
        } else {
    ...
    DelegatingList1(T t1, T t2) {
        data = newJavaList(2);
        data.add(t1);
        data.add(t2);
    }

    DelegatingList1(T t1, T t2, T t3) {
        data = newJavaList(3);
        data.add(t1);
        data.add(t2);
        data.add(t3);
    ...
    DelegatingList1(T t1, T t2, T t3, T... ta) {
    ...
    private C.List<T> emptyList() {
        if (mutable()) {
            return new DelegatingList1<T>(Nil.LIST);
        } else {
            return Nil.list();
        }
    }
    ...
    private C.List<T> copy(int from, int to) {
        ...
        return new DelegatingList1<T>(data.subList(from, to), !mutable).setMutability(mutable);
    }

    private DelegatingList1<T> setMutability(boolean mutable) {
        if (mutable) {
            unsetFeature(C.Feature.IMMUTABLE);
        } else {
            setFeature(C.Feature.IMMUTABLE);
        }
        return this;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof DelegatingList1) {
            DelegatingList1<?> that = (DelegatingList1<?>)obj;
    ...
    public List<T> subList(int fromIndex, int toIndex) {
        return new DelegatingList1<T>(data.subList(fromIndex, toIndex), true);
    }
    ...
    public C.List<T> takeWhile(_.Function<? super T, Boolean> predicate) {
        DelegatingList1<T> ret = new DelegatingList1<T>();
    ...
    public C.List<T> drop(int n) {
        ...
        return new DelegatingList1<T>(data.subList(n, size));
    }
    ...
    public <R> C.List<R> flatMap(_.Function<? super T, Iterable<R>> mapper) {
        DelegatingList1<R> ret = DelegatingList1.createWithInitialCapacity(data.size() * 2);
        for (T t : data) {
    ...
    public C.Sequence<T> filter(_.Function<? super T, Boolean> predicate) {
        DelegatingList1<T> ret = DelegatingList1.createWithInitialCapacity(data.size() * 2);
     ...
     * {@code DelegatingList1} is created with elements from both this
    ...
    public C.Sequence<T> prepend(C.Sequence<T> seq) {
        if (immutable()) {
            if (C.isImmutable(seq)) {
                ...
                DelegatingList1<T> ret = new DelegatingList1<T>(seq);
    ...
    static <T> DelegatingList1<T> createWithInitialCapacity(int initialCapacity) {
        DelegatingList1<T> ret = new DelegatingList1<T>();
